...
if [ "$1" = 'run' ]; then
	...
	rsync -avz --delete  --progress /home/hybris/bin/* ${HYBRIS_HOME}/bin/custom >>/dev/null
	rsync -avz --delete  --progress /home/hybris/config/* ${HYBRIS_HOME}/config >>/dev/null
#	rsync -avz --delete  --progress ${WORKSPACE}/scripts/* ${HYBRIS_HOME}/scripts >>/dev/null
    ...
    # run hybris commerce suite as user hybris
	 source ./setantenv.sh
    gosu hybris ant clean all deploy
